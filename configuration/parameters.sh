...
function upgrade() {
    ...
    available_modules=(
        lang.ccpp C/C++ $([ "${POJDE_NG_MODULE_CCPP_ENABLED}" = "true" ] && echo on || echo off)
        lang.go Go $([ "${POJDE_NG_MODULE_GO_ENABLED}" = "true" ] && echo on || echo off)
        lang.python Python $([ "${POJDE_NG_MODULE_PYTHON_ENABLED}" = "true" ] && echo on || echo off)
        lang.rust Rust $([ "${POJDE_NG_MODULE_RUST_ENABLED}" = "true" ] && echo on || echo off)
        lang.javascript JavaScript $([ "${POJDE_NG_MODULE_JAVASCRIPT_ENABLED}" = "true" ] && echo on || echo off)
        lang.ruby Ruby $([ "${POJDE_NG_MODULE_RUBY_ENABLED}" = "true" ] && echo on || echo off)
        lang.csharp C# $([ "${POJDE_NG_MODULE_CSHARP_ENABLED}" = "true" ] && echo on || echo off)
        lang.java Java $([ "${POJDE_NG_MODULE_JAVA_ENABLED}" = "true" ] && echo on || echo off)
        lang.julia Julia $([ "${POJDE_NG_MODULE_JULIA_ENABLED}" = "true" ] && echo on || echo off)
        lang.octave Octave $([ "${POJDE_NG_MODULE_OCTAVE_ENABLED}" = "true" ] && echo on || echo off)
        lang.r R $([ "${POJDE_NG_MODULE_R_ENABLED}" = "true" ] && echo on || echo off)
        lang.sql SQL $([ "${POJDE_NG_MODULE_SQL_ENABLED}" = "true" ] && echo on || echo off)
        lang.bash Bash $([ "${POJDE_NG_MODULE_BASH_ENABLED}" = "true" ] && echo on || echo off)
        tool.vim Vim $([ "${POJDE_NG_MODULE_VIM_ENABLED}" = "true" ] && echo on || echo off)
        tool.devops "QEMU, Docker and Kubernetes" $([ "${POJDE_NG_MODULE_DEVOPS_ENABLED}" = "true" ] && echo on || echo off)
        tool.techdocs "Technical Documentation" $([ "${POJDE_NG_MODULE_TECHDOCS_ENABLED}" = "true" ] && echo on || echo off)
        tool.latex "Full LaTeX Support" $([ "${POJDE_NG_MODULE_LATEX_ENABLED}" = "true" ] && echo on || echo off)
        tool.webdev "Web Development" $([ "${POJDE_NG_MODULE_WEBDEV_ENABLED}" = "true" ] && echo on || echo off)
        tool.extensions "Common VSCode Extensions" $([ "${POJDE_NG_MODULE_EXTENSIONS_ENABLED}" = "true" ] && echo on || echo off)
        tool.clis "Common CLIs" $([ "${POJDE_NG_MODULE_CLIS_ENABLED}" = "true" ] && echo on || echo off)
        tool.networking Networking $([ "${POJDE_NG_MODULE_NETWORKING_ENABLED}" = "true" ] && echo on || echo off)
        tool.inettui "Browsers and Mail (TUI)" $([ "${POJDE_NG_MODULE_INETTUI_ENABLED}" = "true" ] && echo on || echo off)
        tool.inetgui "Browsers and Mail (GUI)" $([ "${POJDE_NG_MODULE_INETGUI_ENABLED}" = "true" ] && echo on || echo off)
        tool.multimedia Multimedia $([ "${POJDE_NG_MODULE_MULTIMEDIA_ENABLED}" = "true" ] && echo on || echo off)
    )
